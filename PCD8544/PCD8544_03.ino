/**
  Drawing bars (lines) with parameters from serial port
*/
...
void drawLine(uint8_t len, uint8_t thickness);
...
void loop() {
  ...
  uint8_t len, thickness, xAddr, yAddr;
  String str;
  if(0 != Serial.available() )
  {
    str = Serial.readString();
    sscanf(str.c_str(), "%d, %d, %d, %d", &xAddr, &yAddr, &thickness, &len);
    Serial.println(xAddr);
    Serial.println(yAddr);
    Serial.println(len);
    Serial.println(thickness);
    setAddr(xAddr, yAddr);
    drawLine(len, thickness);
  }
}
